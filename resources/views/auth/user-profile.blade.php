            ...
            <div class="form-group">
                ...
                <div>
                    <div class="form-check form-check-inline">
                        <input name="gender" class="form-check-input" type="radio" id="male" value="male" @if($user->profile->gender == 'male') checked @endif>
                        <label class="form-check-label" for="male">Male</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input name="gender" class="form-check-input" type="radio" id="female" value="female" @if($user->profile->gender == 'female') checked @endif>
                ...
                <select name="country_code" id="country_code" class="form-control">
                    ...
                    @foreach(Symfony\Component\Intl\Countries::getNames() as $code => $country)
                        @if($code == 'IL')
                            @continue
                        @endif
                        <option value="{{ $code }}" @if($user->profile->country_code == $code) selected @endif>{{ $country }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="col-6">
            <div class="form-group">
                ...
                <select name="locale" id="locale" class="form-control">
                    <option value="">Select Langueg..</option>
                    @foreach(Symfony\Component\Intl\Locales::getNames() as $code => $locale)
                        <option value="{{ $code }}" @if($user->profile->locale == $code) selected @endif>{{ $locale }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="col-6">
            <div class="form-group">
                ...
                <select name="timezone" id="timezone" class="form-control">
                    ...
                    @foreach(Symfony\Component\Intl\Timezones::getNames() as $code => $timezone)
                        <option value="{{ $code }}" @if($user->profile->timezone == $code) selected @endif>{{ $timezone }}</option>
                    @endforeach
                </select>
            </div>
        </div>
